...
static int display_recommendScore(People *me, People *other) {
	...
	if (strlen(me->type) == 0 || strlen(me->love_type) == 0 ||
		...
		int diff =
			me->age > other->age ? me->age - other->age : other->age - me->age;
		int score = 80 - diff * 3;
...
void display_showHome(SDL_Ui *ui, People *me) {
	...
	Object t_record = gui_initObject(
		...
		(ObjectParam){.text = {"커플 공간", ui->font_normal, COLOR_DURTYPINK}});
	Object t_recordsub =
		gui_initObject(ui, TEXT, card2_x + 26, 522, TOPLEFT,
					   (ObjectParam){.text = {"공유 캘린더와 버킷리스트",
											  ui->font_small, COLOR_GRAY}});
...
void display_showBFS(SDL_Ui *ui, People *me, MatchingInfo *infos, int n) {
	...
	int recommend_count = display_collectRecommendations(
		me, loaded_people, loaded_count, recommendations, recommend_scores, 4);
	...
	for (int i = 0; i < incoming_count; i++) {
		incoming_people[i] = display_findPeopleById(
			loaded_people, loaded_count, incoming_requests[i].from_id);
	}
...
static void s_drawSidebar(SDL_Ui *ui, int active) {
	...
	s_drawText(ui, "로그아웃", ui->font_normal,
			   logout_hover ? COLOR_WHITE : COLOR_DURTYPINK, 70, logout_y + 13,
			   TOPLEFT, 0);
}
...
static void s_openMstSpace(MstCoupleSpace *space, People *me,
						   const char partner_buf[], int *opened, char status[],
						   size_t status_size) {
	if (partner_buf[0] == '\0') {
...
void display_showMST(SDL_Ui *ui, People *me) {
	...
	if (partner_buf[0] != '\0') {
		...
		strcpy(status,
			   "아직 수락된 매칭이 없어요. 매칭 화면에서 제안을 수락해주세요.");
	...
	while (!ui->quit) {
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDL_KEYDOWN:
				if (event.key.keysym.sym == SDLK_BACKSPACE) {
					...
					focus =
						(focus == MST_FOCUS_NONE || focus >= MST_FOCUS_BUCKET)
							? MST_FOCUS_EVENT_DATE
							: focus + 1;
		...
		if (ui->is_mouse_down) {
			...
			if (s_inRect(ui->mx, ui->my, event_x + 28, input_y, 130, 52)) {
				...
				for (int i = 0; i < space.bucket_count && i < 5; i++) {
					if (s_inRect(ui->mx, ui->my, bucket_x + 28, list_y + i * 34,
								 380, 28)) {
		...
		s_drawText(ui, status, ui->font_small, COLOR_SUPERPINK, SV_MAIN_X, 214,
				   TOPLEFT, 0);
		...
		for (int i = 0; i < space.event_count && i < 5; i++) {
			...
			s_drawText(ui, line, ui->font_small, COLOR_DURTYPINK, event_x + 28,
					   list_y + i * 34, TOPLEFT, panel_w - 56);
		}
		...
		s_drawInput(ui, bucket_x + 28, input_y, 260, 52,
					...
					"한강 피크닉");
		int bucket_hover =
			s_inRect(ui->mx, ui->my, bucket_x + 28, add_y, 170, 52);
		s_drawRound(ui, bucket_x + 28, add_y, 170, 52, 14,
					bucket_hover ? COLOR_DURTYPINK : COLOR_VIOLET);
		s_drawText(ui, "버킷 추가", ui->font_small, COLOR_WHITE, bucket_x + 113,
				   add_y + 26, CENTER, 0);
		...
		for (int i = 0; i < space.bucket_count && i < 5; i++) {
			char line[160];
			int hover = s_inRect(ui->mx, ui->my, bucket_x + 28, list_y + i * 34,
								 380, 28);
