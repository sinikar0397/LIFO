...
People *people_create_people(char name[], char id[], char pw[], char type[],
	...
	if (newPeople == NULL) {
		printf("[ERROR] file : people.c, function : people_create_people.     "
			   "Can't malloc structure.\n");
		return NULL;
	}
...
void people_set_people_gen(People *P, enum Gender gen) { P->gen = gen; }
void people_set_people_age(People *P, int age) { P->age = age; }
...
People *people_read_people(const char path[], int offset) {
	...
	People *resultPeople =
		people_create_people(name, id, "\0", type, love_type, gen, age);
	...
	if (cJSON_IsString(love_attach_item) &&
		love_attach_item->valuestring != NULL)
		people_set_people_love_attach(resultPeople,
									  love_attach_item->valuestring);
...
People **people_read_all_people(int *count) {
	...
	for (int i = line_cnt - 1; i >= 0; i--) {
		...
		if (cJSON_IsString(love_lang_item) &&
			love_lang_item->valuestring != NULL)
